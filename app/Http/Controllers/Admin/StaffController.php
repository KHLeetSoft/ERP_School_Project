...
class StaffController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = Staff::where('school_id', auth()->user()->school_id ?? 1);
    
        // 🔍 Search filter (DataTables ka search)
        ...
        // 🔍 Employment type filter (safe for single/multiple)
        ...
        // ⚡ Agar request AJAX hai to DataTables JSON return karo
        if ($request->ajax()) {
            return DataTables::of($query)
                ...
                ->addColumn('hire_date', function($row) {
                    return $row->hire_date ? $row->hire_date->format('d M Y') : '-';
                })
    
                // Experience calculation
                ->addColumn('experience', function($row) {
                    if (!$row->hire_date) return '-';
                    $years = $row->hire_date->diffInYears(now());
                    return $years . ' year' . ($years != 1 ? 's' : '');
                })
    
                // Employment type safe handling
                ->addColumn('employment_type', function($row) {
                    if (is_array($row->employment_type)) {
                        return implode(', ', $row->employment_type);
                    }
                    if (is_object($row->employment_type)) {
                        return json_encode($row->employment_type);
                    }
                    return (string) $row->employment_type;
                })
    
                // Actions buttons
                ->addColumn('actions', function($row) {
                    return view('admin.hr.staff.partials.actions', compact('row'))->render();
                })
        ...
        // Agar AJAX nahi hai → normal blade view ke liye data bhejo
        ...
        return view('admin.hr.staff.index', compact('staff', 'departments', 'designations', 'employmentTypes'));
    }
