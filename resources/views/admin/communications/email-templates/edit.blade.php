                ...
                <form id="templateForm">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" id="category" name="category" required>
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category }}" {{ $template->category == $category ? 'selected' : '' }}>
                                            {{ ucfirst($category) }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="subject" name="subject" required 
                               value="{{ $template->subject }}" placeholder="Enter email subject">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="content" name="content" rows="12" required 
                                  placeholder="Enter email content. Use variables like {{student_name}} to personalize your message.">{{ $template->content }}</textarea>
                    ...
                    <div class="text-center mt-4">
                        <a href="{{ route('admin.communications.email-templates.show', $template->id) }}" 
                           class="btn btn-secondary btn-lg mr-3">
                            <i class="fas fa-eye mr-1"></i> View Template
                        </a>
                        <button type="button" class="btn btn-warning btn-lg mr-3" onclick="saveAsDraft()">
...
function updateTemplate() {
    ...
    $.ajax({
        url: '{{ route("admin.communications.email-templates.update", $template->id) }}',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(response) {
            if (response.success) {
                ...
                window.location.href = '{{ route("admin.communications.email-templates.show", $template->id) }}';
            } else {
                toastr.error(response.message || 'Failed to update template');
            }
        },
        error: function(xhr) {
