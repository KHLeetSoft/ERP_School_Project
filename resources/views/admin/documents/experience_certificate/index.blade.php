...
<div class="container-fluid">
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h4 class="mb-0">
                <i class="bx bx-briefcase me-2 text-primary"></i> Experience Certificates (Employees)
            </h4>
            <div class="btn-group" role="group">
                <a href="{{ route('admin.documents.experience-certificate.create') }}" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus me-1"></i> Create
                </a>
                <a href="{{ route('admin.documents.experience-certificate.dashboard') }}" class="btn btn-info btn-sm">
                  <i class="bx bx-bar-chart"></i> Dashboard
                </a>
                <a href="{{ route('admin.documents.experience-certificate.export') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm border-0">
        <div class="table-responsive p-3">
            <table id="ecTable" class="table table-striped align-middle w-100">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Import Experience Certificates</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.documents.experience-certificate.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
$(function () {
    $('#ecTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.documents.experience-certificate.index') }}",
        dom:
    ...
    $(document).on('click', '.delete-ec-btn', function() {
        let action = $(this).data('action');
        if(confirm('Are you sure to delete this Experience Certificate?')) {
