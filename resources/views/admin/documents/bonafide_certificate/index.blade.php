...
<div class="container-fluid">
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h4 class="mb-0">
                <i class="bx bx-id-card me-2 text-primary"></i> Bonafide Certificates
            </h4>
            <div class="btn-group" role="group">
                <a href="{{ route('admin.documents.bonafide-certificate.create') }}" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus me-1"></i> Create
                </a>
                <a href="{{ route('admin.documents.bonafide-certificate.dashboard') }}" class="btn btn-info btn-sm">
                  <i class="bx bx-bar-chart"></i> Dashboard
                </a>
                <a href="{{ route('admin.documents.bonafide-certificate.export') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm border-0">
        <div class="table-responsive p-3">
            <table id="bcTable" class="table table-striped align-middle w-100">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Import Bonafide Certificates</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.documents.bonafide-certificate.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
$(function () {
    $('#bcTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.documents.bonafide-certificate.index') }}",
        dom:
    ...
    $(document).on('click', '.delete-bc-btn', function() {
        let action = $(this).data('action');
        if(confirm('Are you sure to delete this Bonafide Certificate?')) {
            $.post(action, {_method:'DELETE', _token:'{{ csrf_token() }}'}, function(){
                $('#bcTable').DataTable().ajax.reload();
            });
        }
    });
});
