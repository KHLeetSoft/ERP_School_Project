...
<div class="container-fluid">
    ...
    <div class="card mt-4 shadow-sm"><div class="card-header fw-bold">Recent Employee Conduct Certificates</div><div class="card-body p-0"><div class="table-responsive"><table class="table table-striped mb-0"><thead><tr><th>#</th><th>ECC No.</th><th>Employee</th><th>Conduct</th><th>Issue Date</th><th>Status</th><th></th></tr></thead><tbody>@forelse($recent as $r)<tr><td>{{ $r->id }}</td><td>{{ $r->ecc_number }}</td><td>{{ $r->employee_name }}</td><td>{{ $r->conduct }}</td><td>{{ optional($r->issue_date)->format('Y-m-d') }}</td><td>{{ ucfirst($r->status) }}</td><td><a href="{{ route('admin.documents.employee-conduct-certificate.show', $r) }}" class="btn btn-sm btn-outline-secondary">View</a></td></tr>@empty<tr><td colspan="7" class="text-center text-muted">No records</td></tr>@endforelse</tbody></table></div></div></div>
</div>
@endsection
