...
<div class="container-fluid">
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h4 class="mb-0">
                <i class="bx bx-book-alt me-2 text-primary"></i> Study Certificates
            </h4>
            <div class="btn-group" role="group">
                <a href="{{ route('admin.documents.study-certificate.create') }}" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus me-1"></i> Create
                </a>
                <a href="{{ route('admin.documents.study-certificate.dashboard') }}" class="btn btn-info btn-sm">
                  <i class="bx bx-bar-chart"></i> Dashboard
                </a>
                <a href="{{ route('admin.documents.study-certificate.export') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm border-0">
        <div class="table-responsive p-3">
            <table id="scTable" class="table table-striped align-middle w-100">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Import Study Certificates</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.documents.study-certificate.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
$(function () {
    $('#scTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.documents.study-certificate.index') }}",
        dom:
    ...
    $(document).on('click', '.delete-sc-btn', function() {
        ...
        if(confirm('Are you sure to delete this Study Certificate?')) {
            $.post(action, {_method:'DELETE', _token:'{{ csrf_token() }}'}, function(){
                $('#scTable').DataTable().ajax.reload();
            });
        }
    });
});
