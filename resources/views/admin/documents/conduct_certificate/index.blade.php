...
<div class="container-fluid">
    <div class="card shadow-sm mb-4 border-0">
        <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
            <h4 class="mb-0">
                <i class="bx bx-shield-quarter me-2 text-primary"></i> Conduct Certificates (Students)
            </h4>
            <div class="btn-group" role="group">
                <a href="{{ route('admin.documents.conduct-certificate.create') }}" class="btn btn-primary btn-sm">
                    <i class="fas fa-plus me-1"></i> Create
                </a>
                <a href="{{ route('admin.documents.employee-conduct-certificate.index') }}" class="btn btn-dark btn-sm">
                    <i class="bx bx-user"></i> Employee CC
                </a>
                <a href="{{ route('admin.documents.conduct-certificate.dashboard') }}" class="btn btn-info btn-sm">
                  <i class="bx bx-bar-chart"></i> Dashboard
                </a>
                <a href="{{ route('admin.documents.conduct-certificate.export') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm border-0">
        <div class="table-responsive p-3">
            <table id="ccTable" class="table table-striped align-middle w-100">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Import Conduct Certificates</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <form action="{{ route('admin.documents.conduct-certificate.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
...
$(function () {
    $('#ccTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.documents.conduct-certificate.index') }}",
        dom:
    ...
    $(document).on('click', '.delete-cc-btn', function() {
        ...
        if(confirm('Are you sure to delete this Conduct Certificate?')) {
            $.post(action, {_method:'DELETE', _token:'{{ csrf_token() }}'}, function(){
                $('#ccTable').DataTable().ajax.reload();
            });
        }
    });
});
