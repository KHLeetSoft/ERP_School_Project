    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('admin.users.accountants.update', $accountant->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    <option value="1" {{ old('status', $accountant->status) == 1 ? 'selected' : '' }}>Active</option>
                                    <option value="0" {{ old('status', $accountant->status) == 0 ? 'selected' : '' }}>Inactive</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mt-3">
                            <div class="col-12">
                                ...
                                <a href="{{ route('admin.users.accountants.index') }}" class="btn btn-secondary">Cancel</a>
                            </div>
                        </div>
                    </form>
                </div>
