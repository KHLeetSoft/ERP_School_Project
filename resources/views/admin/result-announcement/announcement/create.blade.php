...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h4 class="mb-0"><i class="bx bx-bullhorn me-2 text-primary"></i>Create Result Announcement</h4>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.result-announcement.announcement.store') }}">
                @csrf
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('announcement_type') is-invalid @enderror" 
                                            ...
                                            <option value="exam_result" {{ old('announcement_type') == 'exam_result' ? 'selected' : '' }}>Exam Result</option>
                                            <option value="online_exam_result" {{ old('announcement_type') == 'online_exam_result' ? 'selected' : '' }}>Online Exam Result</option>
                                            <option value="general_result" {{ old('announcement_type') == 'general_result' ? 'selected' : '' }}>General Result</option>
                                            <option value="merit_list" {{ old('announcement_type') == 'merit_list' ? 'selected' : '' }}>Merit List</option>
                                        </select>
                            ...
                            <div class="mb-3" id="online_exam_div" style="display: none;">
                                ...
                                <select class="form-select @error('online_exam_id') is-invalid @enderror" 
                                    ...
                                    @foreach($onlineExams as $exam)
                                        <option value="{{ $exam->id }}" {{ old('online_exam_id') == $exam->id ? 'selected' : '' }}>
                                            {{ $exam->title }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="target_audience[]" 
                                           ...
                                           {{ in_array('teachers', old('target_teachers', [])) ? 'checked' : '' }}>
                                        ...
                                        <select class="form-select" id="section_ids" name="section_ids[]" multiple>
                                            @foreach($sections as $section)
                                                <option value="{{ $section->id }}" 
                                                        {{ in_array($section->id, old('section_ids', [])) ? 'selected' : '' }}>
                                                    {{ $section->name }}
                                                </option>
                                            @endforeach
                                        </select>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="form-check mb-2">
                                <input class="form-check-input" type="checkbox" name="send_push_notification" 
                                       value="1" id="send_push_notification" {{ old('send_push_notification') ? 'checked' : '' }}>
                ...
                <div class="text-end">
                    <button type="submit" class="btn btn-primary">
                        <i class="bx bx-save me-1"></i> Create Announcement
                    </button>
                </div>
            </form>
        </div>
