...
<div class="container-fluid">
	<div class="card">
		<form id="bulk-delete-form" method="POST" action="{{ route('admin.library.returns.bulk-delete') }}" onsubmit="return confirm('Delete selected returns?')">
			@csrf
			<div class="card-header d-flex justify-content-between align-items-center">
				<h4 class="mb-0"><i class="fas fa-undo-alt me-2 text-primary"></i>Book Returns</h4>
				<div class="btn-group" role="group">
					<a href="{{ route('admin.library.returns.create') }}" class="btn btn-primary btn-sm">
						<i class="fas fa-plus me-1"></i> Add Return
					</a>
					<a href="{{ route('admin.library.returns.export') }}" class="btn btn-success btn-sm">
						<i class="fas fa-file-export me-1"></i> Export CSV
					</a>
					<button type="button" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#importModal">
						<i class="fas fa-file-import me-1"></i> Import CSV
					</button>
					<button type="submit" form="bulk-form" class="btn btn-outline-danger btn-sm" onclick="return confirm('Delete selected returns?')">
			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="returns-table" class="table table-striped table-bordered table-hover align-middle w-100">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="importModalLabel">Import Returns</h5>
					<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
				</div>
				<form method="POST" action="{{ route('admin.library.returns.import') }}" enctype="multipart/form-data">
					@csrf
					<div class="modal-body">
						...
						<small class="text-muted">Expected columns: book_issue_id, returned_at, condition(optional), fine_paid(optional), remarks(optional)</small>
					</div>
	...
	<div class="modal fade" id="deleteReturnModal" tabindex="-1" aria-hidden="true">
	  <div class="modal-dialog">
		<div class="modal-content">
		  ...
		  <div class="modal-body">Are you sure you want to delete this return?</div>
		  <div class="modal-footer">
			<button class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
			<form id="returnDeleteForm" method="POST" action="">
			  @csrf @method('DELETE')
...
function loadTable() {
	...
	table = $('#returns-table').DataTable({
		processing: true,
		serverSide: true,
		order: [[3, 'desc']],
		ajax: { url: "{{ route('admin.library.returns.index') }}", type: 'GET', data: function(d){ d.student_name = document.getElementById('filter-student-name').value; } },
		    dom:
...
$(document).ready(function(){
	...
	$(document).on('click', '.delete-return-btn', function() {
		...
		const actionUrl = `{{ url('admin/library/returns') }}/${id}`;
		$('#returnDeleteForm').attr('action', actionUrl);
		const modal = new bootstrap.Modal(document.getElementById('deleteReturnModal'));
		modal.show();
	});
	$('#select-all').on('change', function(e){
		$('#returns-table tbody input.row-select').prop('checked', this.checked);
	});
	$('#bulk-delete-form').on('submit', function(){
		this.querySelectorAll('input[name="ids[]"]').forEach(el => el.remove());
		$('#returns-table tbody input.row-select:checked').each((_, el) => {
