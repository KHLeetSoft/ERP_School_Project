...
<div class="card">
  <div class="card-header d-flex justify-content-between align-items-center">
    <h4 class="mb-0">Student Promotions</h4>
    <a href="{{ route('admin.students.promotions.create') }}" class="btn btn-sm btn-primary rounded-pill">
...
function loadPromotionTable(){
  ...
  table = $('#promotions-table').DataTable({
    ...
    serverSide: true,
    ajax: {
      url: "{{ route('admin.students.promotions.index') }}",
      data: function(d){
    ...
    columns: [
      ...
      { data: 'from_class', name: 'fromClass.name' },
      { data: 'to_class', name: 'toClass.name' },
      { data: 'promoted_at', name: 'promoted_at' },
...
$(document).ready(function(){
  loadPromotionTable();
  $('#applyFilter').on('click', function(){ loadPromotionTable(); });
  $(document).on('click','.delete-promotion', function(){
    const id = $(this).data('id');
    if (!confirm('Delete this promotion?')) return;
    $.ajax({
      url: `{{ url('admin/students/promotions') }}/${id}`,
      method: 'DELETE',
