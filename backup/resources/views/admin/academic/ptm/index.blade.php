...
<div class="card shadow-sm mb-4 border-0">
    <div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
        <h4 class="mb-0">
            <i class="fas fa-handshake me-2 text-primary"></i> PTM Management
        </h4>
        <div class="btn-group" role="group">
            <a href="{{ route('admin.academic.ptm.create') }}" class="btn btn-primary btn-sm">
                <i class="fas fa-plus me-1"></i> Create
            </a>
            <a href="{{ route('admin.academic.ptm.dashboard') }}"  class="btn btn-info me-2">
                <i class="bx bx-bar-chart"></i> Dashboard
            </a>
            <a href="{{ route('admin.academic.ptm.export') }}" class="btn btn-success btn-sm">
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <table id="ptm-datatable" class="table table-striped table-hover align-middle w-100">
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            ...
            <div class="modal-body text-center">
                <p class="fs-6">Are you sure you want to delete this PTM?</p>
            </div>
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                <form id="deletePtmForm" method="POST" action="#">
                    @csrf
                    @method('DELETE')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title"><i class="fas fa-file-csv me-2"></i> Import PTM (CSV)</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                ...
                <form id="importForm" method="POST" action="{{ route('admin.academic.ptm.import') }}" enctype="multipart/form-data">
                    @csrf
...
<script>
$(function(){
    const table = $('#ptm-datatable').DataTable({
        processing:true,
        serverSide:true,
        responsive:true,
        ajax:{ url:'{{ route('admin.academic.ptm.index') }}', type:'GET' },
        dom:
        ...
        columns:[
            ...
            {data:'end_time', name:'end_time'},
            {
                ...
                render:function(data){
                    let badgeClass = data === 'active' ? 'success' : (data === 'pending' ? 'warning' : 'secondary');
    ...
    // Delete flow
    $(document).on('click', '.delete-ptm-btn', function(){
        const action = $(this).data('action');
        $('#deletePtmForm').attr('action', action);
