...
<div class="container-fluid">
	<div class="card shadow-sm mb-4 border-0">
		<div class="card-header bg-light d-flex justify-content-between align-items-center py-3">
			<h4 class="mb-0"><i class="bx bx-plus me-2 text-primary"></i> Add Question</h4>
	...
	<form method="POST" action="{{ route('admin.online-exam.questions.store') }}" id="questionForm">
		@csrf
		<div class="row">
			<div class="col-lg-8">
							...
							<div class="col-md-6 mb-3">
								...
								<select name="type" id="type" class="form-select" required>
									@foreach(['mcq' => 'MCQ', 'true_false' => 'True/False', 'short_answer' => 'Short Answer', 'essay' => 'Essay'] as $key=>$label)
										<option value="{{ $key }}" {{ old('type','mcq') == $key ? 'selected' : '' }}>{{ $label }}</option>
									@endforeach
								</select>
							</div>
							<div class="col-md-6 mb-3">
								...
								<select name="difficulty" class="form-select">
									@foreach(['easy','medium','hard'] as $d)
										<option value="{{ $d }}" {{ old('difficulty','medium') == $d ? 'selected' : '' }}>{{ ucfirst($d) }}</option>
									@endforeach
								</select>
							</div>
						</div>
						...
						<div id="mcqOptions" style="display: none;">
							...
							<div class="row g-2">
								@foreach(['A','B','C','D'] as $opt)
									<div class="col-md-6">
										<input type="text" name="options[{{ $opt }}]" class="form-control" placeholder="Option {{ $opt }}" value="{{ old('options.'.$opt) }}">
									</div>
								@endforeach
							</div>
						</div>
				...
				<div class="card shadow-sm border-0 mb-4">
					...
					<div class="card-body">
						<div class="mb-3">
							...
							<input type="number" name="marks" class="form-control" min="1" value="{{ old('marks', 1) }}" required>
						</div>
						<div class="mb-3">
