...
class FeeStructureController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $query = FeeStructure::with(['class', 'school'])
                ->where('fee_structures.school_id', auth()->user()->school_id ?? 1)
                ->orderByDesc('fee_structures.created_at');

            return DataTables::of($query)
                ->addIndexColumn()
                ->addColumn('actions', function ($r) {
                    return view('admin.finance.fee-structure.partials.actions', compact('r'))->render();
                })
                ->editColumn('class.name', function ($r) {
                    return $r->class->name ?? 'N/A';
                })
                ->editColumn('amount', function ($r) {
                    return $r->formatted_amount;
                })
                ->editColumn('late_fee', function ($r) {
                    return $r->formatted_late_fee;
                })
                ->editColumn('max_discount', function ($r) {
                    return $r->formatted_max_discount;
                })
                ->editColumn('frequency', function ($r) {
                    return $r->frequency_label;
                })
                ->editColumn('due_date', function ($r) {
                    return $r->due_date ? $r->due_date->format('d M Y') : 'N/A';
                })
                ->editColumn('is_active', function ($r) {
                    return $r->status_badge;
                })
        ...
        return view('admin.finance.fee-structure.index', compact('classes'));
    }
    ...
    public function create()
    {
        ...
        return view('admin.finance.fee-structure.create', compact('classes', 'academicYears', 'feeTypes', 'frequencies'));
    }
